...
Measures initial impulse leg (A to B), waits for 50% retracement to point C, then projects second leg (C to D) equal in length to AB. Enter trade at point D completion when CD equals AB length.
...
class AbcdHarmonicPattern(BaseStrategy):
    name = "abcd_harmonic_pattern"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"abcd_harmonic_pattern_{'bull' if direction == 1 else 'bear'}",
            )
