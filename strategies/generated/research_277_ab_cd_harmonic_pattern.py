...
Measure an aggressive impulse move from point A to B. Wait for price to retrace exactly 50% of that length to point C. Enter trade at point D where the second leg (C to D) equals the length of the first leg (A to B). Enter short at D in an uptrend or long at D in a downtrend.
...
class Research277AbCdHarmonicPattern(BaseStrategy):
    name = "research_277_ab_cd_harmonic_pattern"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if True:
            ...
            tag = "research_277_ab_cd_harmonic_pattern_bull" if direction == 1 else "research_277_ab_cd_harmonic_pattern_bear"
            return Signal(
                direction=direction,
