...
import numpy as np
import logging
_log = logging.getLogger('strategy.parabolic_short')
...
class ParabolicShort(BaseStrategy):
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if self._diag_count % 12 == 0:
            lookback = self.get_param("lookback")
            closes = self.closes(lookback)
            mean_val = float(np.mean(closes)) if len(closes) >= lookback else 0
            _log.info(
                f"DIAG #{self._diag_count}: close={candle.close:.0f} "
                f"zscore={zscore:.2f} thresh={self.get_param('zscore_threshold')} "
                f"parabolic={is_parabolic} spike_state={self._spike_detected} "
                f"mean={mean_val:.0f} prev_high={self._prev_high:.0f}"
            )

        # State machine: detect spike, then wait for lower high
        if is_parabolic and not self._spike_detected:
            _log.info(
                f"PARABOLIC SPIKE detected: zscore={zscore:.2f} close={candle.close:.0f} "
                f"waiting for lower high confirmation"
            )
        ...
        if self._spike_detected:
            ...
            if self.get_param("use_lower_high_confirm"):
                if candle.high >= self._prev_high:
                    ...
                    _log.debug(f"SPIKE: still higher highs ({candle.high:.0f} >= {self._prev_high:.0f})")
                    self._prev_high = candle.high
                    ...
                    if not is_parabolic:
                        ...
                        if current_z < self.get_param("zscore_threshold") * 0.7:
                            _log.info(f"SPIKE RESET: zscore decayed to {current_z:.2f}")
                            self._spike_detected = False
                    return Signal(direction=None)

                # Lower high confirmed! Short entry.
                _log.info(f"LOWER HIGH CONFIRMED: {candle.high:.0f} < {self._prev_high:.0f}, entering short")
            else:
