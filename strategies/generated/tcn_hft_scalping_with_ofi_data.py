#!/usr/bin/env python3
"""
TCN HFT Scalping with OFI Data
==============================
Auto-generated from: i built an HFT trading bot from home with ai
Category: scalping
Confidence: 90%

High-frequency scalping strategy using a Temporal Convolutional Network (TCN) machine learning model with 73.3% area under curve and 56.56% win rate. The bot consumes Order Flow Imbalance (OFI) data from Binance websocket (including velocity, acceleration, and multi-level weighted OFI) to predict short-term price movements, entering long positions when model confidence exceeds 0.55 and exiting based on fixed profit/loss targets or time limits.
...
class TcnHftScalpingWithOfiData(BaseStrategy):
    name = "tcn_hft_scalping_with_ofi_data"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"tcn_hft_scalping_with_ofi_data_{'bull' if direction == 1 else 'bear'}",
            )
