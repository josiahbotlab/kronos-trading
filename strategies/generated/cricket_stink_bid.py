#!/usr/bin/env python3
"""
Cricket Stink Bid (First Inning Mean Reversion)
===============================================
Auto-generated from: Claude Code + Cricket + Polymarket = The Edge Nobody Knows About
Category: mean_reversion
Confidence: 90%

Places lowball limit bids 30% below current market price on the favorite team in Cricket T20 matches (IPL and PSL leagues) exclusively during the first inning. All orders are cancelled once the second inning begins (the 'chase'). The strategy exploits high uncertainty and potential whale panic-selling during the first half to acquire positions at a discount, holding until market expiration for settlement.
...
class CricketStinkBid(BaseStrategy):
    name = "cricket_stink_bid"
    version = "0.1"

    def default_params(self) -> dict:
        return {
            "bid_discount_percent": 30,
            "cooldown_bars": 5,
            "entry_strength": 0.8,
            "liq_percentile": 85,
            "max_history": 300,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"cricket_stink_bid_{'bull' if direction == 1 else 'bear'}",
            )
