#!/usr/bin/env python3
"""
Smart DCA with SMA-Volume Filtering
===================================
Research ID: 28
Category: scalping
Confidence: 85%

A sophisticated dollar-cost averaging bot that adjusts accumulation aggression based on price position relative to the 20-day SMA. When price is below the 20-day SMA, it buys aggressively at fixed intervals during specific trading windows. When price is above the 20-day SMA, it switches to selective buying only on significant intraday dips or near daily lows, confirmed by volume spikes.
...
class Research28SmartDcaWithSmaVolumeFiltering(BaseStrategy):
    name = "research_28_smart_dca_with_sma_volume_filtering"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if True and vol_ratio >= self.get_param('vol_mult'):
            ...
            tag = "research_28_smart_dca_with_sma_volume_filtering_bull" if direction == 1 else "research_28_smart_dca_with_sma_volume_filtering_bear"
            return Signal(
                direction=direction,
