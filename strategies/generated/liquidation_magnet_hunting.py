#!/usr/bin/env python3
"""
Liquidation Magnet Hunting
==========================
Auto-generated from: ai agents that actually trade live (+$1,156 pnl)
Category: momentum
Confidence: 95%

A momentum strategy based on the thesis that market makers push price toward liquidation clusters to maximize liquidations ('liquidation magnets'). The strategy identifies proximity to large short liquidation levels (1-2% away) for bullish signals, but waits for actual long liquidations (capitulation) to occur before entering, effectively taking over liquidated positions. Uses dual monitoring to invalidate if market conditions shift (new short liquidations appear).
...
class LiquidationMagnetHunting(BaseStrategy):
    name = "liquidation_magnet_hunting"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"liquidation_magnet_hunting_{'bull' if direction == 1 else 'bear'}",
            )
