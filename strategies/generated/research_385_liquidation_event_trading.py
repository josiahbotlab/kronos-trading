#!/usr/bin/env python3
"""
Liquidation Event Trading (Big Lick Strategy)
=============================================
Research ID: 385
Category: breakout
Confidence: 75%

Trade based on the thesis that crypto price moves between liquidation levels and is 'least real' during large liquidation events. The strategy triggers when mass liquidations occur, creating temporary price dislocations or pivot points that can be exploited by automated systems.
...
class Research385LiquidationEventTrading(BaseStrategy):
    name = "research_385_liquidation_event_trading"
    version = "0.1"

    def default_params(self) -> dict:
        return {
            "cooldown_bars": 12,
            "drawdown_tolerance_percent": 1,
            "entry_strength": 0.8,
            "liq_percentile": 85,
            "liquidation_threshold_usd": 10000000,
            "max_history": 500,
            "max_hold_bars": 60,
            "take_profit_pct": 3.0,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            tag = "research_385_liquidation_event_trading_bull" if direction == 1 else "research_385_liquidation_event_trading_bear"
            return Signal(
                direction=direction,
