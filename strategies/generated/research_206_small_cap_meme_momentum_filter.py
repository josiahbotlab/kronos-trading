#!/usr/bin/env python3
"""
Small Cap Meme Momentum Filter
==============================
Research ID: 206
Category: momentum
Confidence: 70%

A filtering approach for new token launches that excludes tokens with extremely low liquidity (e.g., $100 market cap) and tokens already pumped significantly (high 24h % change), focusing instead on early-stage tokens with sufficient but not excessive volume.
...
class Research206SmallCapMemeMomentumFilter(BaseStrategy):
    name = "research_206_small_cap_meme_momentum_filter"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        # Entry condition
        if vol_ratio >= self.get_param('vol_mult'):
            ...
            tag = "research_206_small_cap_meme_momentum_filter_bull" if direction == 1 else "research_206_small_cap_meme_momentum_filter_bear"
            return Signal(
                direction=direction,
