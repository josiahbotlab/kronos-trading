#!/usr/bin/env python3
"""
Liquidation Cascade Contrarian
==============================
Auto-generated from: Results of Actual AI Applied To Trading
Category: reversal
Confidence: 85%

A contrarian mean-reversion strategy that enters long positions when significant buy liquidations occur (indicating longs being liquidated/cascading stops) and short positions when significant sell liquidations occur. The logic is based on backtested data showing that liquidation cascades often mark local exhaustion points. Originally implemented with machine learning classification (Random Forest/LSTM) but grounded in the liquidation threshold signal.
...
class LiquidationCascadeContrarian(BaseStrategy):
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active and rsi_ready and (current_rsi > 70 or current_rsi < 30):
            ...
            return Signal(
                ...
                tag=f"liquidation_cascade_contrarian_{'bull' if direction == 1 else 'bear'}",
            )
