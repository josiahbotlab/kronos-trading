#!/usr/bin/env python3
"""
Trailing Stop Momentum Protection
=================================
Auto-generated from: this trading bot makes it so hard to lose long term
Category: momentum
Confidence: 90%

Replaces fixed take-profit targets with a dynamic trailing stop that locks in profits while allowing winners to run. The stop follows the price at a set distance (e.g., 5% below highest price for longs) and only moves in favorable directions, closing the position only when momentum reverses.
...
class TrailingStopMomentumProtection(BaseStrategy):
    name = "trailing_stop_momentum_protection"
    version = "0.1"

    def default_params(self) -> dict:
        return {
            "cooldown_bars": 5,
            "disables_fixed_take_profit": True,
            "entry_strength": 0.8,
            "liq_percentile": 85,
            "max_history": 300,
            "max_hold_bars": 20,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"trailing_stop_momentum_protection_{'bull' if direction == 1 else 'bear'}",
            )
