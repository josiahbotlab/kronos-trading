#!/usr/bin/env python3
"""
Liquidation-Based Mean Reversion (Hide Behind Liquidations)
===========================================================
Auto-generated from: Trade Automatically Without Emotions (Here's How)
Category: mean_reversion
Confidence: 70%

Monitor large leveraged positions on exchanges to identify liquidation levels. Wait for a large counterparty (e.g., a $17 million long position) to be liquidated when price moves against them (e.g., 2% below entry), causing a forced-selling spike. Enter a position in the opposite direction of the liquidation (e.g., go long after longs are liquidated) to capture the mean reversion bounce from the temporary price dislocation.
...
class LiquidationBasedMeanReversion(BaseStrategy):
    name = "liquidation_based_mean_reversion"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active and rsi_ready and (current_rsi > 70 or current_rsi < 30):
            ...
            return Signal(
                ...
                tag=f"liquidation_based_mean_reversion_{'bull' if direction == 1 else 'bear'}",
            )
