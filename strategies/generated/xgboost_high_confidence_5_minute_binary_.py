#!/usr/bin/env python3
"""
XGBoost High-Confidence 5-Minute Binary Prediction
==================================================
Auto-generated from: Polymarket + Claude Code + Machine Learning = Money Printer
Category: momentum
Confidence: 85%

Train an XGBoost machine learning model on 100 weeks of 1-minute BTC candle data to predict 5-minute directional moves on Polymarket binary markets. Only execute trades when the model outputs high confidence signals (approximately >55% confidence), which historically filters 60,000 possible trading windows down to ~143 selective trades with elevated win rates.
...
class XgboostHighConfidence5MinuteBinary(BaseStrategy):
    name = "xgboost_high_confidence_5_minute_binary_"
    version = "0.1"
    ...
    def default_params(self) -> dict:
        return {
            "cooldown_bars": 5,
            "entry_strength": 0.8,
            "high_confidence_trade_count": 143,
            "liq_percentile": 85,
            "max_history": 300,
            "max_hold_bars": 20,
            "take_profit_pct": 5.0,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"xgboost_high_confidence_5_minute_binary__{'bull' if direction == 1 else 'bear'}",
            )
