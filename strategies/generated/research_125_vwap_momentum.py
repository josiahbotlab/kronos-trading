...
A trend-following strategy that enters long positions in Ethereum when the price is above the Volume Weighted Average Price (VWAP) and exits/sells when the price drops below the VWAP.
...
class Research125VwapMomentum(BaseStrategy):
    name = "research_125_vwap_momentum"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if True and vol_ratio >= self.get_param('vol_mult'):
            ...
            tag = "research_125_vwap_momentum_bull" if direction == 1 else "research_125_vwap_momentum_bear"
            return Signal(
                direction=direction,
