...
Uses a systematic approach to position sizing starting at $2 per trade, with plans to increase size gradually (by $1-2 weekly) if performance is positive. Includes a 'double rule' where size doubles when using filtered high-quality wallet lists, with the ability to scale back down if performance degrades.
...
class GraduatedPositionSizing(BaseStrategy):
    name = "graduated_position_sizing"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if ma_ready:
            ...
            return Signal(
                ...
                tag=f"graduated_position_sizing_{'bull' if direction == 1 else 'bear'}",
            )
