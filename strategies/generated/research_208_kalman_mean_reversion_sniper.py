#!/usr/bin/env python3
"""
Kalman Mean Reversion Sniper
============================
Research ID: 208
Category: mean_reversion
Confidence: 80%

Short-only mean reversion strategy using Kalman filter to create a dynamic fair value baseline. Enters short positions when price deviates significantly above the Kalman fair value estimate, catching overextended moves likely to snap back.
...
class Research208KalmanMeanReversionSniper(BaseStrategy):
    name = "research_208_kalman_mean_reversion_sniper"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if (current_rsi > self.get_param('rsi_ob') or current_rsi < self.get_param('rsi_os')):
            ...
            tag = "research_208_kalman_mean_reversion_sniper_bull" if direction == 1 else "research_208_kalman_mean_reversion_sniper_bear"
            return Signal(
                direction=direction,
