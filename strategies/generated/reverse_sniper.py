#!/usr/bin/env python3
"""
Reverse Sniper (Short to Zero)
==============================
Auto-generated from: we can now short meme coins to 0..
Category: mean_reversion
Confidence: 90%

Short-selling strategy targeting the statistical certainty that 99% of meme coins eventually go to zero. Uses lending protocols to borrow tokens and sell them immediately, aiming to repurchase at significantly lower prices as the asset depreciates toward zero.
...
class ReverseSniper(BaseStrategy):
    name = "reverse_sniper"
    version = "0.1"

    def default_params(self) -> dict:
        return {
            "borrow_immediately_sell": True,
            "cooldown_bars": 5,
            "entry_strength": 0.8,
            "liq_percentile": 85,
            "max_history": 300,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"reverse_sniper_{'bull' if direction == 1 else 'bear'}",
            )
