#!/usr/bin/env python3
"""
Multi-Account Liquidation-Controlled Scalping
=============================================
Auto-generated from: Copying The $8 Billion Fund's Trading Bot (Hudson River Trading)
Category: scalping
Confidence: 80%

A risk-filtered scalping strategy that runs multiple bot instances across different accounts with varying risk parameters. One version incorporates liquidation data as a trade filter (preventing entries during high liquidation periods), while another trades continuously. Uses maker order execution with strict rate limiting (60 requests/minute for standard accounts, 1 second between calls).
...
class MultiAccountLiquidationControlledSca(BaseStrategy):
    name = "multi_account_liquidation_controlled_sca"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        # Entry condition
        if cascade_active and rsi_ready:
            ...
            return Signal(
                ...
                tag=f"multi_account_liquidation_controlled_sca_{'bull' if direction == 1 else 'bear'}",
            )
