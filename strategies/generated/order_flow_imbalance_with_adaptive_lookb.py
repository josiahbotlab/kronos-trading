#!/usr/bin/env python3
"""
Order Flow Imbalance (OFI) with Adaptive Lookback Periods
=========================================================
Auto-generated from: Copying The $8 Billion Fund's Trading Bot (Hudson River Trading)
Category: momentum
Confidence: 85%

A high-frequency market making strategy that calculates Order Flow Imbalance using multi-level order book data (top 5 bid/ask levels) with dynamically adjusting lookback windows based on real-time volatility. The strategy adapts to market conditions by shortening lookback periods during high volatility and extending them during low volatility to capture order book pressure more accurately.
...
class OrderFlowImbalanceWithAdaptiveLookb(BaseStrategy):
    name = "order_flow_imbalance_with_adaptive_lookb"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"order_flow_imbalance_with_adaptive_lookb_{'bull' if direction == 1 else 'bear'}",
            )
