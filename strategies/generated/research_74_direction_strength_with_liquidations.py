...
A momentum-based scalping strategy that uses direction strength indicators combined with liquidation data to determine trade entries. The strategy monitors liquidation volumes over the last 10 minutes and pauses trading when liquidations exceed a threshold, presumably to avoid choppy or high-volatility conditions.
...
class Research74DirectionStrengthWithLiquidations(BaseStrategy):
    name = "research_74_direction_strength_with_liquidations"
    version = "0.1"
    ...
    def default_params(self) -> dict:
        return {
            ...
            "entry_strength": 0.8,
            "liq_percentile": 85,
            "liquidation_threshold": 250000000,
            "max_history": 500,
            "max_hold_bars": 60,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        # Entry condition
        if vol_ratio >= self.get_param('vol_mult') and cascade_active:
            ...
            tag = "research_74_direction_strength_with_liquidations_bull" if direction == 1 else "research_74_direction_strength_with_liquidations_bear"
            return Signal(
                direction=direction,
