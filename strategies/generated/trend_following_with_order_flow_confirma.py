#!/usr/bin/env python3
"""
Trend-Following with Order Flow Confirmation
============================================
Auto-generated from: Claude Code Just Changed Algorithmic Trading Forever
Category: momentum
Confidence: 85%

A momentum strategy designed for high volatility periods. Instead of pausing during ATR spikes, the bot identifies trend direction using 5-minute price change and a 20-period SMA on the 1-minute chart. It then places small test orders (bid and ask) to detect immediate buy/sell pressure. It enters long only if trend is up and ask fills first (buy pressure), or short only if trend is down and bid fills first (sell pressure). Misaligned signals trigger a cooldown instead of entry.
...
class TrendFollowingWithOrderFlowConfirma(BaseStrategy):
    name = "trend_following_with_order_flow_confirma"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if ma_ready:
            ...
            return Signal(
                ...
                tag=f"trend_following_with_order_flow_confirma_{'bull' if direction == 1 else 'bear'}",
            )
