#!/usr/bin/env python3
"""
CVD (Cumulative Volume Delta) Tick Data Strategy
================================================
Auto-generated from: Polymarket 5 Min Claude Code Bot are NUTS
Category: momentum
Confidence: 70%

Uses Cumulative Volume Delta calculated from tick-by-tick data (intra-bar) rather than OHLCV candles to detect order flow pressure on 5-minute Polymarket markets. Strategy identifies aggressive buyers (rising CVD, lifting the ask) versus aggressive sellers (falling CVD, hitting the bid) by summing buy volume minus sell volume cumulatively for every trade execution.
...
class CvdTickDataStrategy(BaseStrategy):
    name = "cvd_tick_data_strategy"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"cvd_tick_data_strategy_{'bull' if direction == 1 else 'bear'}",
            )
