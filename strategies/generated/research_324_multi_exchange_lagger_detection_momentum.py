#!/usr/bin/env python3
"""
Multi-Exchange Lagger Detection Momentum
========================================
Research ID: 324
Category: momentum
Confidence: 95%

Monitors real-time price feeds across multiple perpetual exchanges (Aster, Hyperliquid, Habachi, Extended, Pacifica) via websockets to detect price leadership and lag. When a leader exchange shows significant percentage price movement (divergence) that hasn't yet appeared on Aster (the trading venue), the bot takes a directional trade on Aster expecting it to catch up. Combines lagger detection with range filtering to avoid buying at local tops or selling at local bottoms.
...
class Research324MultiExchangeLaggerDetectionMomentum(BaseStrategy):
    name = "research_324_multi_exchange_lagger_detection_momentum"
    version = "0.1"
    ...
    def default_params(self) -> dict:
        return {
            ...
            "max_history": 500,
            "max_hold_bars": 60,
            "minimum_divergence_bps": 50,
            "range_lookback_bars": 15,
            "slow_period": 30,
            "take_profit_pct": 3.0,
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if True:
            ...
            tag = "research_324_multi_exchange_lagger_detection_momentum_bull" if direction == 1 else "research_324_multi_exchange_lagger_detection_momentum_bear"
            return Signal(
                direction=direction,
