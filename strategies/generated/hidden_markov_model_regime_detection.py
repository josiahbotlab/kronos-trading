...
Uses HMM to infer hidden market states (regimes) from observable price patterns. The model learns transition probabilities between 2-4 hidden states (e.g., 80% probability trending regime persists, 20% switch to choppy) based on return and volatility data to determine current market regime.
...
class HiddenMarkovModelRegimeDetection(BaseStrategy):
    name = "hidden_markov_model_regime_detection"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if rsi_ready:
            ...
            return Signal(
                ...
                tag=f"hidden_markov_model_regime_detection_{'bull' if direction == 1 else 'bear'}",
            )
