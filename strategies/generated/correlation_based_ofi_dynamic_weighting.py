#!/usr/bin/env python3
"""
Correlation-Based OFI Dynamic Weighting
=======================================
Auto-generated from: Correlation Based OFI Feature Selection 4 Trading Bots
Category: scalping
Confidence: 85%

A high-frequency scalping strategy using dynamically weighted Order Flow Imbalance (OFI) features to predict ultra-short-term price movements (5-30 seconds). Feature weights are recalculated hourly based on Pearson correlations to forward returns, giving highest weight to Depth/Balance and Z-score while ignoring volume and acceleration. Trades only in low volatility regimes when composite correlation scores exceed thresholds.
...
class CorrelationBasedOfiDynamicWeighting(BaseStrategy):
    name = "correlation_based_ofi_dynamic_weighting"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"correlation_based_ofi_dynamic_weighting_{'bull' if direction == 1 else 'bear'}",
            )
