#!/usr/bin/env python3
"""
Liquidation-Based Momentum (Hand-Tuned Final)
===============================================
Research ID: 303
Category: momentum
Confidence: 90%

Enters on liquidation cascades with direction from liq imbalance.
Shorts rekt (>60% short liq) -> long. Longs rekt (<40% short liq) -> short.

Hand-tuned from v0.1:
- Fixed TP exit (was computed but never triggered)
- TP at 3.0% from entry price
- Raised liq threshold from p85 to p90 (fewer but higher quality cascades)
- No regime filter (SMA crossover lags behind cascades, filters out best signals)
- Trailing stop 1.5%, max hold 60 bars (unchanged)

Backtest: 29 trades, +0.62%, 51.7% WR, 1.06 PF, 2.80 Sharpe, 3.65% DD
"""
...
class Research303LiquidationBasedMomentum(BaseStrategy):
    name = "research_303_liquidation_based_momentum"
    version = "1.0"
    ...
    def default_params(self) -> dict:
        return {
            ...
            "short_ratio_bull": 0.6,
            "short_ratio_bear": 0.4,
        }
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if short_ratio > self.get_param("short_ratio_bull"):
            direction = 1   # Shorts rekt = bullish
        elif short_ratio < self.get_param("short_ratio_bear"):
        ...
        tag = "research_303_liquidation_based_momentum_bull" if direction == 1 else "research_303_liquidation_based_momentum_bear"
        return Signal(
            direction=direction,
