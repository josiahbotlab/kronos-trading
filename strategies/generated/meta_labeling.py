...
Machine learning overlay that takes base signals (like liquidation events) and classifies them as trade/no-trade using a secondary classifier trained with triple barrier method (take profit, stop loss, max holding time) to filter false positives and size positions by confidence probability.
...
class MetaLabeling(BaseStrategy):
    name = "meta_labeling"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"meta_labeling_{'bull' if direction == 1 else 'bear'}",
            )
