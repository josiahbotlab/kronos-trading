...
import numpy as np
import logging
_log = logging.getLogger('strategy.obv_divergence')
...
class ObvDivergence(BaseStrategy):
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        # Diagnostic: log every 12 candles (~1 hour)
        if self._diag_count % 12 == 0:
            rsi_val = self.rsi(self.get_param("rsi_period"))
            lookback = self.get_param("lookback_bars")
            closes = self.closes(lookback) if len(self._candle_history) >= lookback else np.array([])
            obvs = np.array(self._obv_history[-lookback:]) if len(self._obv_history) >= lookback else np.array([])
            n_pivot_lows = len(self._find_pivot_lows(closes, self.get_param("pivot_bars"))) if len(closes) > 0 else 0
            n_pivot_highs = len(self._find_pivot_highs(closes, self.get_param("pivot_bars"))) if len(closes) > 0 else 0
            _log.info(
                f"DIAG #{self._diag_count}: close={candle.close:.0f} "
                f"div={div} pivotL={n_pivot_lows} pivotH={n_pivot_highs} "
                f"rsi={'%.1f' % rsi_val if rsi_val else 'N/A'} "
                f"obv_len={len(self._obv_history)} "
        ...
        # Log divergence detection (important)
        _log.info(f"DIVERGENCE DETECTED dir={div} close={candle.close:.0f}")
        ...
        if self.get_param("use_rsi_confirm"):
            ...
            if rsi_val is not None:
                if div == 1 and rsi_val > self.get_param("rsi_oversold"):
                    _log.info(f"DIVERGENCE BLOCKED by RSI: div=bull rsi={rsi_val:.1f} > oversold={self.get_param('rsi_oversold')}")
                    return Signal(direction=None)
                if div == -1 and rsi_val < self.get_param("rsi_overbought"):
                    _log.info(f"DIVERGENCE BLOCKED by RSI: div=bear rsi={rsi_val:.1f} < overbought={self.get_param('rsi_overbought')}")
                    return Signal(direction=None)
