...
Short-only strategy using Kalman Filter to create a dynamic fair value baseline. Identifies overextended moves likely to snap back by measuring deviation from the filtered trend line.
...
class KalmanMeanReversionSniper(BaseStrategy):
    name = "kalman_mean_reversion_sniper"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        if cascade_active:
            ...
            return Signal(
                ...
                tag=f"kalman_mean_reversion_sniper_{'bull' if direction == 1 else 'bear'}",
            )
