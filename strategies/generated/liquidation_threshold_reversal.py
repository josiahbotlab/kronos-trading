#!/usr/bin/env python3
"""
Liquidation Threshold Reversal
==============================
Auto-generated from: how to actually do algorithmic trading
Category: reversal
Confidence: 85%

A mean-reversion strategy that enters long positions when large-scale liquidation events exceed a specific USD threshold (e.g., $12M), based on the thesis that price becomes 'least real' during forced liquidations (big licks) and tends to revert or move between liquidity levels. The strategy aggregates liquidation data from multiple exchanges (Binance and OKX) across various lookback periods to identify entry points, distinguishing between long and short liquidations to gauge directional bias.
...
class LiquidationThresholdReversal(BaseStrategy):
    name = "liquidation_threshold_reversal"
    version = "0.1"
    ...
    def on_candle(self, candle: CandleData) -> Signal:
        ...
        # Entry condition
        if cascade_active and rsi_ready and (current_rsi > 70 or current_rsi < 30):
            ...
            return Signal(
                ...
                tag=f"liquidation_threshold_reversal_{'bull' if direction == 1 else 'bear'}",
            )
