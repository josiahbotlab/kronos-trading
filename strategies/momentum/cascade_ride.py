...
class CascadeRide(BaseStrategy):
    ...
    def default_params(self) -> dict:
        return {
            # Cascade detection
            "liq_threshold_usd": 5000,     # min USD liquidated to trigger
            "liq_count_min": 1,             # min number of liquidation events
            "liq_ratio_threshold": 0.55,    # 65%+ of liqs on one side = directional cascade

            # Entry
            "use_confirmation": False,        # wait for price to confirm direction
            ...
            # Filters
            "min_volume_sma_ratio": 1.0,     # volume must be 1.2x average
