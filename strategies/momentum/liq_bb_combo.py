...
import numpy as np
import logging
_log = logging.getLogger('strategy.liq_bb_combo')
...
class LiquidationBollingerCombo(BaseStrategy):
    ...
    def _momentum_logic(self, candle: CandleData) -> Signal:
        ...
        # Diagnostic: log key metrics every 12 candles (~1 hour)
        if self._diag_count % 12 == 0:
            liq_values = self.liq_usd(self.get_param("lookback_bars"))
            nonzero = liq_values[liq_values > 0]
            threshold = float(np.percentile(nonzero, self.get_param("liq_percentile"))) if len(nonzero) >= 20 else 0
            bb = self.bollinger_bands(self.get_param("bb_period"), self.get_param("bb_std"))
            bb_str = "upper=%.0f/mid=%.0f/lower=%.0f" % bb if bb else "N/A"
            rsi_val = self.rsi(self.get_param("rsi_period")) if self.get_param("use_rsi") else None
            _log.info(
                f"DIAG #{self._diag_count}: close={candle.close:.0f} "
                f"liq={candle.liquidation_usd:.0f} liq_thresh={threshold:.0f} "
                f"nz_liqs={len(nonzero)} bb=[{bb_str}] "
        ...
        # Log cascade detection (rare and important)
        _log.info(
            f"CASCADE DETECTED dir={cascade_dir} close={candle.close:.0f} "
            f"liq_usd={candle.liquidation_usd:.0f}"
        )
