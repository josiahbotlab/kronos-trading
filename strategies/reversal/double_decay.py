...
class DoubleDecayReversal(BaseStrategy):
    ...
    def default_params(self) -> dict:
        return {
            # Cascade detection
            "cascade_percentile": 80,
            "lookback_bars": 300,
            "min_history": 50,
            "liq_direction_threshold": 0.55,

            # Double decay detection
            "decay_1_ratio": 0.7,            # first bar must drop to 50% of cascade
            "decay_2_ratio": 0.5,           # second bar must drop to 25% of cascade
            "max_wait_bars": 8,              # give up if no decay within N bars
            ...
            # Filters
            "require_rsi_extreme": False,     # RSI must be overbought/oversold
            "rsi_period": 14,
            "rsi_overbought": 70,
            "rsi_oversold": 30,
            "cooldown_bars": 8,
