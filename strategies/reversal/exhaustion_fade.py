...
class ExhaustionFade(BaseStrategy):
    ...
    def default_params(self) -> dict:
        return {
            ...
            "cascade_percentile": 80,        # P95+ event triggers watch mode
            "lookback_bars": 300,
            "min_history": 50,

            # Exhaustion detection
            "decay_bars": 1,                 # wait N bars for decay after cascade
            "decay_ratio": 0.5,              # liq must drop to 30% of cascade level
            "price_extension_atr": 2.0,      # price must be 2 ATR from mean

            # Entry
            "use_bb_confirmation": False,      # require price outside Bollinger Bands
            "bb_period": 20,
            "bb_std": 2.0,
